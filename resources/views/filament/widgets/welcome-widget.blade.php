...
<x-filament-widgets::widget class="fi-wi-custom-welcome" :dir="app()->getLocale() === 'ar' ? 'rtl' : 'ltr'">
...
">
        ...
        {{-- ─── Header Row ─── --}}
        <div style="display:flex;align-items:flex-start;justify-content:space-between;flex-wrap:wrap;gap:1.25rem;position:relative;z-index:1;width: 100%;">
            ...
            <div>
                ...
                <p style="font-size:0.9rem;color:rgba(255,255,255,0.7);margin:0.375rem 0 0;">
                    {{ __('widgets.welcome_to_dashboard') }}
                </p>
            </div>

            {{-- Brand badge --}}
            <div style="display:flex;align-items:center;gap:0.75rem;background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.2);border-radius:1rem;padding:0.75rem 1.25rem;backdrop-filter:blur(8px);margin-inline-start: auto;">
                ...
                <div>
                    <p style="font-size:0.875rem;font-weight:800;color:#fff;margin:0;line-height:1;">{{ __('widgets.wisal_store') }}</p>
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-top:1.75rem;position:relative;z-index:1;">
            ...
            <div style="background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.18);border-radius:1rem;padding:1.25rem;transition:background 0.2s,transform 0.15s;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.625rem;">
                    <span style="font-size:0.8rem;font-weight:600;color:rgba(255,255,255,0.7);">{{ __('widgets.today_revenue') }}</span>
                ...
                <p style="font-size:1.625rem;font-weight:900;color:#fff;margin:0;line-height:1;">
                    {{ $todayRevenue }}<span style="font-size:0.8rem;font-weight:500;color:rgba(255,255,255,0.65);margin-inline-start:0.25rem;">{{ app(\App\Services\CurrencyService::class)->symbol() }}</span>
                </p>
            </div>
            ...
            <div style="background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.18);border-radius:1rem;padding:1.25rem;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.625rem;">
                    <span style="font-size:0.8rem;font-weight:600;color:rgba(255,255,255,0.7);">{{ __('widgets.today_orders') }}</span>
                    <svg xmlns="http://www.w3.org/2000/svg" style="width:1.25rem;height:1.25rem;color:rgba(255,255,255,0.75);" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                </div>
                <p style="font-size:1.625rem;font-weight:900;color:#fff;margin:0;line-height:1;">
                    {{ $todayOrders }}<span style="font-size:0.8rem;font-weight:500;color:rgba(255,255,255,0.65);margin-inline-start:0.25rem;">{{ __('reports.order_unit') }}</span>
                </p>
            </div>
            ...
            <div style="background:rgba(255,255,255,0.12);border:1px solid rgba(255,255,255,0.18);border-radius:1rem;padding:1.25rem;">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:0.625rem;">
                    <span style="font-size:0.8rem;font-weight:600;color:rgba(255,255,255,0.7);">{{ __('widgets.pending_orders') }}</span>
                    <svg xmlns="http://www.w3.org/2000/svg" style="width:1.25rem;height:1.25rem;color:rgba(255,255,255,0.75);" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <div style="display:flex;align-items:center;gap:0.625rem;flex-wrap:wrap;">
                    ...
                    @if($pendingOrders > 0)
                    <span style="font-size:0.68rem;font-weight:700;background:rgba(253,230,138,0.2);color:#fde68a;border:1px solid rgba(253,230,138,0.35);border-radius:9999px;padding:0.15rem 0.6rem;white-space:nowrap;">{{ __('widgets.needs_review') }}</span>
                    @endif
                </div>
            </div>
